...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type === $customerRoleID) {
            if (empty($data['from_language_id'])) {
                return $this->errorResponse($response, "from_language_id", "Du måste fylla in alla fält");
            }

            if (empty($data['duration'])) {
                return $this->errorResponse($response, "duration", "Du måste fylla in alla fält");
            }

            if ($data['immediate'] == 'no') {
                if (empty($data['due_date']) || empty($data['due_time'])) {
                    return $this->errorResponse($response, "due_date", "Du måste fylla in alla fält");
                }

                if (empty($data['customer_phone_type']) && empty($data['customer_physical_type'])) {
                    return $this->errorResponse($response, "customer_phone_type", "Du måste göra ett val här");
                }
                ...
                if ($dueCarbon->isPast()) {
                    return $this->errorResponse($response, "", "Can't create booking in past");
                }
            ...
            $data['gender'] = $this->setGender($data['job_for']);
            $data['certified'] = $this->setCertified($data['job_for']);
    ...
    private function errorResponse(&$response, $fieldName, $message) {
    ...
    private function setGender($jobFor) {
    ...
    private function setCertified($jobFor){
        $certification = ''; // Initialize variable to store certification status
        ...
        $jobFor->certified = $certification; // Set the certification status for the job
    }
    ...
    public function storeJobEmail($data)
    {
        ...
        $job->fill($this->getJobFieldsFromData($data));
        $job->save();

        $user = $job->user;
        $recipientEmail = $job->user_email ?? $user->email;
        $recipientName = $user->name;
        $subject = sprintf('Vi har mottagit er tolkbokning. Bokningsnr: #%d', $job->id);
    ...
    /**
     * Get the job fields from the given data.
     *
     * @param array $data The job email data.
     *
     * @return array The job fields.
     */
    private function getJobFieldsFromData(array $data): array
    {
        $fields = [];
        ...
        if ($address !== null || $instructions !== null || $town !== null) {
            $userMeta = $data['user_email'] ? null : $data['user']->userMeta;
            $fields['address'] = $address ?? $userMeta->address ?? null;
            $fields['instructions'] = $instructions ?? $userMeta->instructions ?? null;
            $fields['town'] = $town ?? $userMeta->city ?? null;
        }
    ...
    /**
     * Fire the JobWasCreated event for the given job.
     *
     * @param Job $job The job.
     *
     * @return void
     */
    private function fireJobWasCreatedEvent(Job $job): void
    {
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $job->session_time = $this->calculateSessionTime($job->due, $job->end_at);
        $job->save();
        ...
        $translator = $job->translatorJobRel->firstWhere('completed_at', null)
                                               ->firstWhere('cancel_at', null);
    ...
    private function calculateSessionTime($dueDate, $completedDate)
    {
        ...
        $interval = $completedDateTime->diff($dueDateTime);
        return $interval->format('%h:%i:%s');
    }

    private function formatSessionTime($sessionTime)
    {
        [$hours, $minutes, $seconds] = explode(':', $sessionTime);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        // Determine which type of job to retrieve based on the user's translator type.
        ...
        $userLanguages = UserLanguages::where('user_id', $userId)->pluck('lang_id')->toArray();
        $gender = $userMeta->gender;
        $translatorLevel = $userMeta->translator_level;

        // Retrieve potential job IDs based on the user's criteria.
        $jobIds = Job::getJobs($userId, $jobType, 'pending', $userLanguages, $gender, $translatorLevel);

        // Filter out any jobs that don't match the user's location.
        $jobs = $jobIds->filter(function ($jobId) use ($userId) {
        ...
        // Convert the remaining job IDs to job objects and return the result.
        return TeHelper::convertJobIdsInObjs($jobs->toArray());
    }
